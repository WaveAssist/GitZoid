...
INCREMENTAL_INTRO = "🔄 _New commits detected! Here's an updated review based on the latest changes._\n\n"

# Run output modal: green-toned links (not default blue); selectable URL for webview/iframe copy
OUTPUT_LINK_STYLE = (
    "color: #1b5e20; font-weight: 600; text-decoration: underline; "
...
def generate_full_comment(review_dict: dict) -> str:
    ...
    if potential_optimizations:
        ...
        output_comment += f"## 🚀 Potential Optimizations\n\n{formatted_optimizations}\n\n"
    if suggestions:
    ...
    for pr in prs_to_review:
        ...
        if result:
            ...
            display_output_content += (
                ...
                f'<a href="{safe_href}" target="_blank" rel="noopener noreferrer" style="{OUTPUT_LINK_STYLE}">'
                f"View comment on GitHub</a>"
                f"</div>"
            )
            posted_review_links.append(pr_comment_url)

    if posted_review_links:
        display_output_content += (
            f'<div style="margin-top: 10px; margin-bottom: 14px;">'
            f'<span style="{OUTPUT_URL_HINT_STYLE}">'
            f"If links do not open in this view, copy one of the URLs below.</span>"
            f"</div>"
        )
        for posted_url in posted_review_links:
            display_output_content += f'<span style="{OUTPUT_URL_SELECT_STYLE}">{html.escape(posted_url)}</span>'
